...
public static class RibbonIndexExtensions
{
    ...
    extension(IRibbonIndex x)
    {
        public bool GetRibbonIndex(RibbonIndex idx) => x.GetRibbon((int)idx);
        public void SetRibbonIndex(RibbonIndex idx, bool value = true) => x.SetRibbon((int)idx, value);
    ...
    extension(RibbonIndex r)
    {
        ...
        public void Fix(in RibbonVerifierArguments args, bool state)
        {
            var pk = args.Entity;
        ...
        private void FixEncounterMark(PKM pk, bool state)
        {

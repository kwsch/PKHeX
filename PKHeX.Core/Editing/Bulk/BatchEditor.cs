...
public sealed class BatchEditor
{
    ...
    public bool Process(PKM pk, IEnumerable<StringInstruction> filters, IEnumerable<StringInstruction> modifications, Func<PKM, bool> modifier)
    {
        if (pk.Species == 0)
            return false;
        if (!pk.Valid)
    ...
    public static BatchEditor Execute(ReadOnlySpan<string> lines, IEnumerable<PKM> data, Func<PKM, bool> modifier)
    {

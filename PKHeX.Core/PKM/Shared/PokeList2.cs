...
/// <summary>
/// List of <see cref="PK2"/> prefixed by a count.
/// </summary>
public static class PokeList2
{
    // Structure:
    ...
    public const byte SlotEgg = 0xFD;

    /// <summary>
    /// Gets the species identifier for the slot.
    /// </summary>
    public static byte GetHeaderIdentifierMark(PK2 pk)
    {
    ...
    private static bool IsEgg(byte marker) => marker == SlotEgg;
    ...
    /// </summary>
    public static PK2 ReadFromSingle(ReadOnlySpan<byte> input)
    {
        var jp = IsJapaneseList(input.Length);
    ...
    public static PK2 ReadFromList(ReadOnlySpan<byte> input, int stringLength, int capacity = 1, bool isParty = true, int index = 0)
    {
        ...
        return new PK2(body, stringSection1, stringSection2) { IsEgg = IsEgg(input[1 + index]) };
    }
    ...
    /// <inheritdoc cref="WrapSingle(PK2,Span{byte})"/>
    public static byte[] WrapSingle(PK2 pk)
    {

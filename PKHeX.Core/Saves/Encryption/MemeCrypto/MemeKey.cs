...
public readonly ref struct MemeKey
{
    ...
    /// <summary> AES Initialization Vector (IV) </summary>
    private readonly byte[] IV; // Added to store the IV
    ...
    public MemeKey(MemeKeyIndex key)
    {
        ...
        // Initialize IV to null; it will be set in GetAesImpl when needed
        IV = null;
    }
    ...
    private void AesDecrypt(ReadOnlySpan<byte> data, Span<byte> sig)
    {
        ...
        for (var i = sig.Length - chunk; i >= 0; i -= chunk)
        {
            ...
            Xor(temp, slice);
            aes.DecryptCbc(temp, IV, temp); // Use the stored IV
            temp.CopyTo(slice);
        }
    ...
    private void AesEncrypt(Span<byte> data, Span<byte> sig)
    {
        ...
        for (var i = 0; i < sig.Length; i += chunk)
        {
            var slice = sig.Slice(i, chunk);
            Xor(slice, temp);
            aes.EncryptCbc(slice, IV, slice); // Use the stored IV
            slice.CopyTo(temp);
        }
    ...
    private IAesCryptographyProvider.IAes GetAesImpl(ReadOnlySpan<byte> payload)
    {
        ...
        // Initialize IV to zero for now; will be updated dynamically.
        // Since IV can't be set on the IAes interface, store it in the struct
        unsafe
        {
            fixed (byte* ivPtr = IV)
            {
                if (ivPtr == null) // First time setting the IV
                {
                    IV = new byte[aes.BlockSize / 8]; // 16 bytes for AES
                    // Zero-initialize the IV (as per the original comment)

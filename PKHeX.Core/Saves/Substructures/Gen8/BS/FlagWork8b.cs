...
public sealed class FlagWork8b : SaveBlock<SAV8BS>, IEventFlag, ISystemFlag, IEventWork<int>
{
    ...
    private int GetOffsetSystem(int index)
    {
        ...
        return Offset + OFS_SYSTEM + (4 * index);
    }

    private int GetOffsetWork(int index)
    {
        if ((uint)index >= COUNT_WORK)
            throw new ArgumentOutOfRangeException(nameof(index), $"Expected a number below {COUNT_WORK}, not {index}.");
        return Offset + OFS_WORK + (4 * index);
    }

    public bool GetFlag      (int index) => ReadInt32LittleEndian(Data.AsSpan(GetOffsetFlag(index))) == 1;
    public bool GetSystemFlag(int index) => ReadInt32LittleEndian(Data.AsSpan(GetOffsetSystem(index))) == 1;
    public int GetWork       (int index) => ReadInt32LittleEndian(Data.AsSpan(GetOffsetWork(index)));
    public float GetFloatWork(int index) => ReadSingleLittleEndian(Data.AsSpan(GetOffsetWork(index)));

    public void SetFlag      (int index,  bool value) => WriteUInt32LittleEndian(Data.AsSpan(GetOffsetFlag(index)), value ? 1u : 0u);
    public void SetSystemFlag(int index,  bool value) => WriteUInt32LittleEndian(Data.AsSpan(GetOffsetSystem(index)), value ? 1u : 0u);
    public void SetWork      (int index,   int value) => WriteInt32LittleEndian(Data.AsSpan(GetOffsetWork(index)), value);
    public void SetFloatWork (int index, float value) => WriteSingleLittleEndian(Data.AsSpan(GetOffsetWork(index)), value);

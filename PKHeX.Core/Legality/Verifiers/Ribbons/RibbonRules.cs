...
public static class RibbonRules
{
    ...
    public static (byte Contest, byte Battle) GetMaxMemoryCounts(EvolutionHistory evos, PKM pk, IEncounterTemplate enc)
    {
        ...
        if (evos.HasVisitedGen3)
        {
            ...
            var battle = IsAllowedBattleFrontier(head.Species) || ParseSettings.Settings.Game.Gen3.AllowBattleTowerTeamSwap ? IsRibbonValidWinning(pk, enc, evos) ? MaxBattleBoth : MaxBattleBothNoWinning : (byte)0;

...
public static class EncounterMovesetGenerator
{
    ...
    private static IEnumerable<IEncounterable> GenerateVersionEncounters(PKM pk, ReadOnlyMemory<ushort> moves, GameVersion version, bool egg = true)
    {
        ...
        foreach (var enc in GenerateVersionEncounters(pk, moves, version, generation, context, egg))
            yield return enc;
    ...
    private static IEnumerable<IEncounterable> GetEggs(PKM pk, ReadOnlyMemory<ushort> needs, EvoCriteria[] chain, GameVersion version, IEncounterGenerator generator)
    {
        ...
        {
            if (needs.Length == 0 || (HasAllNeededMovesEgg(needs.Span, egg) && EggMoveVerifier.IsEggMoveCombinationValid(needs.Span, pk.Species, pk.Form, version)))
                yield return egg;
        }
    }

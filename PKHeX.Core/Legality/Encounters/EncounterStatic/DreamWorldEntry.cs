...
namespace PKHeX.Core
{
    ...
    internal record DreamWorldEntry(int Species, int Level, int Move1 = 0, int Move2 = 0, int Move3 = 0, int Form = 0, int Gender = -1)
    {
        ...
        private void AddTo(EncounterStatic5[] result, ref int ctr)
        {
            ...
            result[ctr++] = Create(a, Move1);
            if (Move2 == 0)
                return;
            result[ctr++] = Create(a, Move2);
            if (Move3 == 0)
                return;
            result[ctr++] = Create(a, Move3);
        }

        private EncounterStatic5 Create(int ability, int move)
        {
            return new()
            {
                Species = Species,
        ...
        public static EncounterStatic5[] GetArray(IReadOnlyList<DreamWorldEntry> t)
        {

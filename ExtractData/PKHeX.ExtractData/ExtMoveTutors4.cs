...
namespace PKHeX.ExtractData
{
    class ExtMoveTutors4
    {
        ...
        static string[] moves_tutors_all_hgss = {
            "Air Cutter ",
            "Bug Bite ",
            "Dive ",
            "Fire Punch ",
        ...
        static string[] moves_tutors_all_pt = {
            /*"Blast Burn ",
            "Draco Meteor ",
            "Frenzy Plant ",
            "Hydro Cannon ",*/
        ...
        const string Pt_Folder = "\\Platinum\\";

        internal ushort[] GetMoveTutorsTable(string[] tutors_names)
        {
            string[] all_moves = Util.getMovesList("en");
            ushort[] id_moves_tutors_all = new ushort[tutors_names.Length];

            for (int i = 0; i < id_moves_tutors_all.Length; i++)
            {
                string move_tutor = tutors_names[i].Trim();
                if (all_moves.Contains(move_tutor))
                {
                    id_moves_tutors_all[i] = (ushort)all_moves.
                            Select((move, index) => new { index = index, move = move }).
                            Where(x => x.move == move_tutor).
                            Select(x => x.index).
                            First();
                }
                else
            ...
            return id_moves_tutors_all.OrderBy(x => x).ToArray();
        }

...
namespace PKHeX.WinForms
{
    public partial class DonutFlavorProfile9a : UserControl
    {
        ...
        public void LoadFromDonut(Donut9a donut)
        {
            ...
            DonutInfo.RecalculateDonutFlavors(donut, flavorStats);
            ...
            ScaleValue = maxStat < 100 ? 500 : Math.Min(((maxStat + 99) / 100) * 100 + 100, 760);
        ...
        private void DrawStatsPentagon(Graphics g, PointF center, float radiusX, float radiusY)
        {
            // Calculate scaled points based on stat values with individual max per stat
            PointF[] calculatedPoints = new PointF[5];
            float baselineScale = 0.10f;

            for (int i = 0; i < 5; i++)
            {
                // Calculate individual max for this stat
                int statValue = FlavorProfileStats[i];
                ...
                calculatedPoints[i] = new PointF(
                    center.X + (float)(scaledRadiusX * Math.Cos(angle)),
                    center.Y + (float)(scaledRadiusY * Math.Sin(angle))
                );
            }

            // Reorder points for counter-clockwise drawing
            int[] counterClockwiseOrder = [0, 4, 3, 2, 1];

            PointF[] statPoints = new PointF[5];
            for (int i = 0; i < 5; i++)
            {
        ...
        private void UpdateStatLabels(Span<int> flavorStats)
        {

...
{
    // The ad-hoc release asset set, plus signed variants used to verify tier preference.
    ...
    [
        ...
        Asset("PKHeX-Avalonia-osx-arm64-adhoc.zip"),
        Asset("PKHeX-Avalonia-osx-arm64-adhoc.dmg"),
        Asset("PKHeX-Avalonia-osx-x64-adhoc.zip"),
        Asset("PKHeX-Avalonia-osx-x64-adhoc.dmg"),
    ];

    private static ReleaseAsset Asset(string name) => new(name, $"https://example.com/{name}");

    [Fact]
    public void MacOs_prefers_selfsigned_dmg_over_adhoc_dmg_and_never_picks_zip()
    {
    ...
    [Fact]
    public void MacOs_falls_back_to_adhoc_dmg_when_no_selfsigned_or_plain_dmg_exists()
    {
        var selected = UpdateAssetSelector.SelectAsset(RealAssets, "macos", "arm64", InstallKind.MacAppBundle);

        Assert.NotNull(selected);
        Assert.Equal("PKHeX-Avalonia-osx-arm64-adhoc.dmg", selected!.Name);
    }

    [Fact]
    public void MacOs_prefers_plain_dmg_over_adhoc_dmg()
    {
    ...
    public void MacOs_never_selects_the_zip_build()
    {
        ...
        var assets = new[] { Asset("PKHeX-Avalonia-osx-arm64-adhoc.zip") };
